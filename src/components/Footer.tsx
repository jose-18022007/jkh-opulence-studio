    ...
    <div className="max-w-[1200px] mx-auto px-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-8">
        ...
        <div>
          ...
          <div className="flex gap-3 mt-5">
            {[Instagram, Facebook, Youtube, MessageCircle].map((Icon, i) => (
              <a key={i} href="#" className="w-9 h-9 rounded-full border border-white/10 flex items-center justify-center text-white/50 hover:border-gold hover:text-gold transition-all duration-300 cursor-pointer">
                <Icon size={16} />
              </a>
            ))}
          </div>
        ...
        <div>
          ...
          {['Home', 'About Us', 'Our Designs', 'Generate Design', 'Contact Us'].map(l => (
            <a key={l} href="#" className="block font-inter text-[15px] text-white/40 hover:text-gold transition-colors duration-300 mb-3 cursor-pointer">{l}</a>
          ))}
        </div>
        {/* Col 3 */}
        <div>
          <h4 className="font-inter text-sm uppercase tracking-[2px] text-white font-semibold mb-5">Our Services</h4>
          {['TV Unit Design', 'Kitchen Interior', 'Wardrobe Design', 'Ceiling Design', 'Full Home Interior', 'Commercial Interior'].map(l => (
            <a key={l} href="#" className="block font-inter text-[15px] text-white/40 hover:text-gold transition-colors duration-300 mb-3 cursor-pointer">{l}</a>
          ))}
        </div>
        {/* Col 4 */}
